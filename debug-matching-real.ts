...
async function debug() {
  const targetId = '0044adbd-6ce6-483a-ae33-ac0547154664'
  ...
  const { data: sellers } = await supabase.from('sellers').select('id, first_name, last_name, created_by').eq('created_by', targetId)
  ...
  for (const sheet of sheets || []) {
    ...
    if (fetched.success) {
      ...
      console.log(`Headers found: MES=${mesCol}, DN=${dnCol}`)
