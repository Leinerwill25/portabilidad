...
async function debug() {
  ...
  let out = `Results for ${currentMonthName}:\n`
  for (const sheet of sheets) {
    ...
    if (fetched.success) {
      ...
      if (salesCount > 0) {
        ...
        out += `Seller: ${s.first_name} ${s.last_name} | Sales: ${salesCount} | GID: ${gid}\n`
      }
    }
  }
  fs.writeFileSync('sales_debug.txt', out)
  console.log("Written sales_debug.txt")
}
